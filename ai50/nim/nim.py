...
class NimAI:
    ...
    def best_future_reward(self, state):
        ...
        if len(possible_actions) == 0:
            ...
            max_future_reward = -999_999_999
            for action in possible_actions:
                future_reward = self.q.get((tuple(state), action), 0)
    ...
    def choose_action(self, state, epsilon=True):
        ...
        for action in possible_actions:
            if self.q.get((tuple(state), action), 0) == best_future_reward:
                best_action = action
        if not epsilon:
            return best_action
        else:
            if random.random() < self.epsilon:
                random_action = random.choice(list(possible_actions))
